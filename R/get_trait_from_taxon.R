...
#' @param workflow - options are "formula_by_taxon" or "best_fit"
#'  (default = "best_fit")
...
get_trait_from_taxon <- function(data,
  ...
  assert_that(
    is.character(workflow) & (workflow %in% c("formula_by_taxon", "best_fit")),
    msg = paste(
      workflow,
      "must be a character string corresponding to: formula_by_taxon or best_fit"
    )
  )
  ...
  if(workflow == "formula_by_taxon") {

    trait_sel <- dplyr::filter(decision_df, tax_distance <= max_tax_dist)

  } else if(workflow == "best_fit") {

    trait_sel <-
      decision_df |>
      dplyr::filter(best_fit_choice == TRUE) |>
      dplyr::group_by(row) |>
  ...
  trait_sel <- dplyr::select(trait_sel, -recommend, -explanation, -best_fit_choice)
  ...
  if(nrow(trait_sel) > 0) {
    ...
    if ( (trait == "equation") && (workflow == "best_fit") ) {
