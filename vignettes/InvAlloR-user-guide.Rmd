...
You can install `InvAlloR` from GitHub. However, currently, we transitively depend on [terra](https://github.com/rspatial/terra), which may require additional installation steps (GDAL) on your OS. We recommend consulting the install section of their README before proceeding.

In addition, you may also need [GNparser](https://github.com/gnames/gnparser), which is currently a transitive dependency of [bdc](https://brunobrr.github.io/bdc/) which we rely on for cleaning the taxonomic names. To download [GNparser](https://github.com/gnames/gnparser), you can follow the installation instructions [here](https://rdrr.io/github/brunobrr/bdc/f/vignettes/help/installing_gnparser.Rmd).

For reference, we used We used GNparser [v1.11.6](https://github.com/gnames/gnparser/releases/tag/v1.11.6).
...
```
Once you have installed `terra` and `GNparser`, you can use the following code to install the package:
...
The idea with `formula_by_taxon` is for us to now use our discretion to choose appropriate equations. To look in more detail at the output, we can also use the `View()` function:

```{r eval=FALSE}
View(dplyr::filter(ponds_equ1$data, row == 1))
```
